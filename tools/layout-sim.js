...
const PHYS = slice('function clampSpeed()', '/* ================= 描画');
...
function makeSim(layout){
  return new Function(`
    ...
    ${PHYS}
    return {ball,bumpers,flippers,stats,stepPhysics,G};
  `)();
}

/* ---- フリッパーAI：打てる位置に来たら一瞬だけ振る ---- */
function ai(sim, st){
  const b = sim.ball;
  for(let i=0;i<2;i++){
    const f = sim.flippers[i], s = st[i];
    if(s.hold > 0){ s.hold--; f.held = true; if(s.hold===0){ f.held=false; s.cool=5; } continue; }
    if(s.cool > 0){ s.cool--; f.held = false; continue; }
    const zone = b.vy > 0 && b.y > 640 && b.y < 730 &&
      (i===0 ? (b.x > f.px-16 && b.x < f.px+100) : (b.x < f.px+16 && b.x > f.px-100));
    if(zone){ s.hold = 7; f.held = true; } else f.held = false;
  }
}
...
function evaluate(layout, balls = BALLS){
  ...
  for(let n=0;n<balls;n++){
    ...
    const st=[{hold:0,cool:0},{hold:0,cool:0}];
    ...
    while(t < MAXT && !sim.stats.dead && !sim.stats.lane){
      for(const b of sim.bumpers) if(b.cool>0) b.cool -= DT;
      ai(sim, st);
      sim.stepPhysics(DT);
      t += DT; allF++;
...
function score(m){
  return -3.0*m.zeroRate                        // まとに当たらない球は最悪
       + 2.2*m.balance                          // 3つのまとが平等に当たること
       - 0.22*Math.min(m.firstHit, 10)          // 早く当たること
       + band(m.hitsPerMin, 16, 34, 0.9)        // 忙しすぎず暇すぎず
       + band(m.life, 11, 30, 0.6)              // 1球の長さ
       + band(m.topRate, 0.30, 0.75, 0.5)       // まとのある上半分に居る割合
...
function valid(L){
  ...
  for(const b of bs){
    if(b.x-b.r < 26 || b.x+b.r > 394) return false;
    if(b.y-b.r < 70 || b.y+b.r > 545) return false;
    if(Math.hypot(b.x-ARC.x, b.y-ARC.y) + b.r > 221) return false;
  }
  const clear = (a, b, gap) => Math.hypot(a.x-b.x, a.y-b.y) >= a.r+b.r+gap;
  for(let i=0;i<bs.length;i++) for(let j=i+1;j<bs.length;j++) if(!clear(bs[i],bs[j],24)) return false;
  for(const p of ps){
    if(p.x-p.r < 24 || p.x+p.r > 396) return false;
    if(Math.hypot(p.x-ARC.x, p.y-ARC.y) + p.r > 221) return false;
    for(const b of bs) if(!clear(p,b,24)) return false;
  }
...
function randomLayout(){
  const r = P([34,36,38,40,42,44]);
  const shape = P(['down','row','up','stack']);
  const half = R(78, 140);
  let bs;
  if(shape==='down'){ const yt=R(200,320), ys=yt+R(70,150);
    bs=[{x:CX-half,y:yt,r},{x:CX,y:ys,r},{x:CX+half,y:yt,r}]; }
  else if(shape==='up'){ const ys=R(300,420), yt=ys-R(70,140);
    bs=[{x:CX-half,y:ys,r},{x:CX,y:yt,r},{x:CX+half,y:ys,r}]; }
  else if(shape==='row'){ const y=R(200,400);
    bs=[{x:CX-half,y,r},{x:CX,y,r},{x:CX+half,y,r}]; }
  else { const y0=R(190,280), gap=R(2*r+26,130), dx=R(0,60);
    bs=[{x:CX-dx,y:y0,r},{x:CX+dx,y:y0+gap,r},{x:CX-dx,y:y0+2*gap,r}]; }
  const ps=[];
  if(P([0,1,1,1])){ const py=R(250,340), px=R(34,110), pr=P([12,14,16]);
    ps.push({x:px,y:py,r:pr},{x:420-px,y:py,r:pr}); }
  if(P([0,1])){ const py=R(430,530), px=R(48,110), pr=P([11,13,15]);
    ps.push({x:px,y:py,r:pr},{x:420-px,y:py,r:pr}); }
  bs.forEach(b=>{ b.x=Math.round(b.x); b.y=Math.round(b.y); });
  ps.forEach(p=>{ p.x=Math.round(p.x); p.y=Math.round(p.y); });
  return {bumpers:bs, posts:ps, shape};
}

/* ---- 現在の index.html から配置を読む ---- */
function currentLayout(){
  const bumpers = [...src.matchAll(/\{x:\s*(\d+),\s*y:(\d+),\s*r:(\d+),val:0/g)]
    .map(m=>({x:+m[1], y:+m[2], r:+m[3]}));
  const posts = [...src.match(/const posts = \[([^;]+);/)[1]
    .matchAll(/\{x:(\d+),\s*y:(\d+),\s*r:(\d+)\}/g)]
    .map(m=>({x:+m[1], y:+m[2], r:+m[3]}));
  return {bumpers, posts};
}
...
if(cmd === 'check'){
  ...
  out.slice(0,10).forEach((e,i)=>{
    ...
    console.log('    バンパー', JSON.stringify(e.L.bumpers), ' ピン', JSON.stringify(e.L.posts));
  });
  console.log('\n--- 比較：いまの index.html ---');
